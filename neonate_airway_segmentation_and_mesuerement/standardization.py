...
def save_plot(y_name, mode, save_path):
    """
    ...
    """
    if save_path == "":
        save_path = usefull_function.select_location("choose a directory where to save the plot")
...
def plot_compare(df_list, data=None, mode="all", save_path=""):
...
def plot_compare_3(df_list, data = None, std_flag = True ,mode = "all", percent_mode="pa-ch",pa_percent=0,ch_percent=0, units = 'mm', save_path=""):
    num_of_graph = len(df_list)
    ...
    for y_name in ["area"]:
        ...
        save_plot("3_" + y_name + "_" + percent_mode , mode, save_path=save_path)
